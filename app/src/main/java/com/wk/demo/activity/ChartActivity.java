...
public class ChartActivity extends AppCompatActivity
    ...
    private void initUI() {
        ...
        this.wrBtn = findViewById(R.id.wr_btn);
        ...
        this.wrBtn.setOnClickListener(this);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.boll_btn:
                ...
                break;
            case R.id.wr_btn:
                switchModuleType(ModuleType.WR);
                break;
    ...
    private void switchModuleType(ModuleType moduleType) {
        if (chartLayout.switchModuleType(moduleType) > 0) {
    ...
    private void recoveryChartState() {
        if (!Utils.listIsEmpty(chartCache.enableModuleTypes)) {
            ModuleType moduleType = ModuleType.MACD;
            for (ModuleType item : chartCache.enableModuleTypes) {
                switch (item) {
                    case TIME:
                        rbTimeLine.setChecked(true);
                        moduleType = ModuleType.TIME;
                        break;
                    case MACD:
                    default:
                        macdBtn.setChecked(true);
                        moduleType = ModuleType.MACD;
                        break;
                    case RSI:
                        rsiBtn.setChecked(true);
                        moduleType = ModuleType.RSI;
                        break;
                    case KDJ:
                        kdjBtn.setChecked(true);
                        moduleType = ModuleType.KDJ;
                        break;
                    case BOLL:
                        bollBtn.setChecked(true);
                        moduleType = ModuleType.BOLL;
                        break;
                    case WR:
                        wrBtn.setChecked(true);
                        moduleType = ModuleType.WR;
                        break;
                }
            }
            chartLayout.switchModuleType(ModuleType.CANDLE, ModuleType.VOLUME, moduleType);
            candleChart.onViewChanged();
        } else {
            rbOneHour.setChecked(true);
            macdBtn.setChecked(true);
            chartLayout.switchModuleType(ModuleType.CANDLE, ModuleType.VOLUME, ModuleType.MACD);
            candleChart.onViewChanged();
        }
        ...
        this.candleChart.post(() -> candleChart.getRender().toTransX(chartCache.beginPosition));
