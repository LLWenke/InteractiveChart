...
public class DepthSelectorDrawing extends AbsDrawing<DepthRender> {
  ...
  public void onDrawOver(Canvas canvas) {
    ...
    for (SelectorItemEntry item : selectorInfo) {
      float textWidth = item.getLablePaint().measureText(item.getLable())
    ...
    for (SelectorItemEntry item : selectorInfo) {
      ...
      canvas.drawText(item.getLable(), x, y, item.getLablePaint());
      //绘制unit
  ...
  private void loadSelectorInfo() {
    ...
    //价格
    selectorInfo[0]
        .setLable(attribute.context.getString(R.string.wk_price))
        .setLablePaint(labelPaint)
        .setValue(point.getPrice().text)
    ...
    //总量
    selectorInfo[1]
        .setLable(attribute.context.getString(R.string.wk_total_amount))
        .setLablePaint(labelPaint)
    ...
    //总成本
    selectorInfo[2]
        .setLable(attribute.context.getString(R.string.wk_total_cost))
        .setLablePaint(labelPaint)
