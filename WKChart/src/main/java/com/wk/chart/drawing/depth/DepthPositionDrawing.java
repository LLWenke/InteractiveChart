...
public class DepthPositionDrawing extends AbsDrawing<DepthRender, DepthModule> {
    ...
    public void onDraw(Canvas canvas, int begin, int end, float[] extremum) {
        ...
        String diffLabel = render.getAdapter().rateConversion(askLow.result - bidHigh.result, scale, false, true);
        ...
        canvas.drawText(bidHigh.text, left - bidPriceWidth - labelMargin, top - scaleLineOffset + labelHeightHalf, labelPaint);
        canvas.drawText(askLow.text, right + labelMargin, top - scaleLineOffset + labelHeightHalf, labelPaint);
        canvas.drawText(bidLabel, left - rectSizeHalf - bidLabelRect.width() - labelMargin, rectTop + bidLabelRect.height() - bidRectOffset, bidPaint);
        canvas.drawText(askLabel, right + rectSizeHalf + labelMargin, rectTop + askLabelRect.height() - askRectOffset, askPaint);
        canvas.drawRect(left - rectSizeHalf, rectTop, left + rectSizeHalf, rectTop + rectSize, bidPaint);
        canvas.drawRect(right - rectSizeHalf, rectTop, right + rectSizeHalf, rectTop + rectSize, askPaint);
        scalePath.rewind();
        canvas.restore();
    }
