...
import com.wk.chart.stock.FloatChartModule;
...
public abstract class AbsRender<T extends AbsAdapter, A extends BaseAttribute> {
  ...
  private MeasureUtils measureUtils;//计算工具类
  private FloatChartModule floatChartModule;//跨视图组件

  private final List<AbsChartModule> chartModules = new ArrayList<>(); //图表指标列表
   ...
   * 刷新缩放后的Item信息
   */
  public abstract void resetItemInfo();
   ...
   */
  protected abstract void onViewRect();
   ...
   */
  protected abstract void initDrawing(AbsChartModule absChartModule);
  ...
  public void toTransX(int entryIndex) {
    setCurrentTransX(getTransX(attribute.visibleCount, entryIndex) - 1f);
  }
   ...
   * @param entryIndex entry 索引
   */
  protected float getTransX(float visibleCount, float entryIndex) {
   ...
   * @param entryIndex entry 索引
   */
  public float getPointX(int entryIndex) {
   ...
   */
  private void calibrationMapPoints(float[] pts) {
    if (attribute.borderWidth > 0) {
      for (int i = 1; i < pts.length; i += 2) {
        pts[i] -= attribute.borderWidth;
      }
    }
  }
  ...
  protected void postMatrixValue(AbsChartModule chartModule) {
    ...
    final float scaleX = rect.width() / (deltaX == 0 ? adapter.getCount() : deltaX);
    final float scaleY = rect.height() / deltaY;
    matrixValue.reset();
    if (extremum[3] > 0) {
      if (extremum[1] >= 0) {
        matrixValue.postTranslate(-extremum[0], -extremum[1]);
        matrixValue.postScale(scaleX, -scaleY);
        matrixValue.postTranslate(0, rect.bottom);
      } else {
        matrixValue.postScale(scaleX, -scaleY);
        matrixValue.postTranslate(0, rect.top + extremum[3] / deltaY * rect.height());
      }
    } else {
      matrixValue.postTranslate(0, -extremum[3]);
      matrixValue.postScale(scaleX, -scaleY);
      matrixValue.postTranslate(0, rect.top);
    }
    //反向赋值
  ...
  public void addChartModule(AbsChartModule chartModule) {
    ...
    if (chartModule.getChartLevel() == ChartLevel.MAIN) {
   ...
   * 指标集合
   */
  public List<AbsChartModule> getChartModules() {
    return chartModules;
  }
  ...
  public AbsChartModule getMainChartModule() {
    for (AbsChartModule item : getChartModules()) {
      if (item.getChartLevel() == ChartLevel.MAIN && item.isEnable()) {
        return item;
      }
    }
    return floatChartModule;
